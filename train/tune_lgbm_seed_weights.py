...
def main():
    ...
    pd.DataFrame({"seed": SEEDS, "weight": best_w}).to_csv("result9(537)/weights_lgbm_seeds.csv", index=False)
    print("Saved weights_lgbm_seeds.csv")

    oof_best = blend(oofs, best_w)
    test_best = blend(tests, best_w)

    np.save("result9(537)/oof_proba_lgbm_weighted.npy", oof_best)
    np.save("result9(537)/test_proba_lgbm_weighted.npy", test_best)
